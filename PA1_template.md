...
inital loading of the dataset

```r
setwd("C:/Users/Fiona's Family/Desktop/Coursera/Reproducible Research_Course Project 1")
#Reads the data
...
meanNa = sapply(splitNa, function(x) mean(x$steps))
medNa = sapply(splitNa, function(x) median(x$steps))
```

The following is the adjusted mean values after changing all NA values to 0
...
```
The panel plot is setup to show the differences in the average steps vs time period between weekends and weekdays.

```r
#Creates the plots
par(mfrow=c(2,1))
plot(avgWeek$interval, avgWeek$avgsteps.weekend, type='l', main ="Weekend",
     xlab="Interval", ylab="Number of Steps")
plot(avgWeek$interval, avgWeek$avgsteps.weekday, main ="Weekday", 
